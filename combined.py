...
ssl_steps= 100, # Number of times to generate pseudo-labels
percent_select = 0.5,
optimizer = "Adam",
lr = 1e-5,
...
class Dataset_train_combined(Dataset):    
    def __init__(self, abnormal_feat_dir, normal_feat_dir, unlabelled_feat_dir,
     abnormal_unlabelled_index, normal_unlabelled_index, interpolation_dimension, train_split):   
        ...
        self.concatenated_abnormal_path_dict = dict(list(self.abnormal_UCF_path_dict.items()) + list(self.abnormal_pseudolabel_path_dict.items()))
        self.concatenated_normal_path_dict = dict(list(self.normal_UCF_path_dict.items()) + list(self.normal_pseudolabel_path_dict.items()))
    ...
    def __len__(self):          
        return len(self.concatenated_normal_path_list) 
    ...
    def __getitem__(self, index): 
        ...
        abnormal_video_path = self.concatenated_abnormal_path_list[index]
        normal_video_path = self.concatenated_normal_path_list[index]  
        ...
        return abnormal_segments, abnormal_value, normal_segments, normal_value
...
def train(ssl_step, epoch, model, loader, batch_size, optimizer, device):
    print('\nEpoch: %d' % epoch)
    model.train()
    ...
    if ssl_step == 0:
        ...
        for abnormal_inputs, abnormal_value, normal_inputs, normal_value in tqdm(loader):
            abnormal_inputs = abnormal_inputs.to(device)
            normal_inputs = normal_inputs.to(device)
            abnormal_scores = model(abnormal_inputs) 
            normal_scores = model(normal_inputs)  
    ...
    del loader, abnormal_inputs, normal_inputs, abnormal_scores, normal_scores, combined_anomaly_scores, train_loss, loss
    gc.collect()
...
valid_dataset = Dataset_valid(abnormal_feat_dir, normal_feat_dir, interpolation_dimension, 
                                test_list_combined)
valid_batch = DataLoader(valid_dataset, batch_size=1, 
                              shuffle=True, num_workers=16, pin_memory=True, drop_last=True)

unlabelled_dataset = Dataset_unlabelled(unlabelled_feat_dir, interpolation_dimension)
unlabelled_batch = DataLoader(unlabelled_dataset, batch_size=1, 
                              shuffle=True, num_workers=16, pin_memory=True, drop_last=True)
...
for ssl_step in range(0, ssl_steps):
    ...
    for epoch in range(0, epochs):
        train(ssl_step, epoch, model, train_batch, batch_size, optimizer, device)
        epoch_auc = test(model, valid_batch, interpolation_dimension, device)
    ...
    anomaly_score_dict = unlabelled_predict(model, unlabelled_batch, ssl_step, device)
    abnormal_unlabelled_index, normal_unlabelled_index = unlabelled_select(anomaly_score_dict, 
    percent_select)       

    
    train_ssl_dataset = Dataset_train_combined(abnormal_feat_dir, normal_feat_dir, unlabelled_feat_dir, 
                            abnormal_unlabelled_index, normal_unlabelled_index, 
                            interpolation_dimension, train_split)
    train_batch = DataLoader(train_ssl_dataset, batch_size=batch_size, 
                              shuffle=True, num_workers=16, pin_memory=True, drop_last=True)    
