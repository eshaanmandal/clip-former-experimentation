...
def train(
    ...
    for anomaly, normal in tqdm(zip(anomaly_dl, normal_dl), total=l):
        ...
        if ssl_step != 0 and use_bce:
            try:
                bce_anomaly_loss = sum([bce(score_a[i], 1, a_frac) for i in range(label_a.item()) if label_a[i] == UNLABELLED]) \
                    / torch.numel(label_a[label_a == UNLABELLED])
                loss += bce_anomaly_loss

            except:
                loss += 0 
            try:
                bce_normal_loss = sum([bce(score_n[i], 0, a_frac) for i in range(label_n.item()) if label_n[i] == UNLABELLED]) \
                    / torch.numel(label_n[label_n == UNLABELLED])
